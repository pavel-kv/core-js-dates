...
function getNextFriday(date) {
  ...
  const addDays = dayOfWeek < 5 ? 5 - dayOfWeek : 7 - (dayOfWeek - 5);
...
function formatDate(date) {
  ...
  const sec = newDate.getUTCSeconds();
  let meridiem = '';
  if (hour === 12) {
    meridiem = 'PM';
  } else if (hour > 12) {
  ...
  return `${month}/${day}/${year}, ${hour}:${min.toString().padStart(2, '0')}:${sec.toString().padStart(2, '0')} ${meridiem}`;
}
...
function getWeekNumberByDate(date) {
  const dayOfCurrentDate = date.getUTCDay();
  const startYear = new Date(date.getUTCFullYear(), 0, 1);
  const startDayOfYear = startYear.getUTCDay();
  const period = date - startYear;
  const days = Math.floor(period / (1000 * 60 * 60 * 24)) + 1;
  let weekNumber = Math.ceil((days + startDayOfYear) / 7);
  weekNumber += !startDayOfYear ? 1 : 0;
  weekNumber -= !dayOfCurrentDate ? 1 : 0;
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  do {
    nextDay += 7;
    newDate = new Date(date.getFullYear(), date.getMonth(), nextDay);
    dayFriday = newDate.getDay();
    dateFriday = newDate.getDate();
  } while (dayFriday !== 5 || dateFriday !== 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const convertDate = (date) => date.split('-').reverse();
  ...
  do {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      if (workDate < endDate) {
        workDays.push(getDateFormat(workDate));
      }
      nextDay += 1;
    }
